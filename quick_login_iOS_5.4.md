...
| id                  | 必选 | 2    | string   |如果idtype的值为0，该字段填sourceid,如果idtype的值为1，该字段填appid                        |
| idtype              | 必选 | 2    | string   | 融合版sdk token校验时，该字段填"1"，否则填"0"                           |
...
| sign                | 必选 | 2    | string   | 当**encryptionalgorithm≠"RSA"**时，sign = MD5（appid + version + msgid + systemtime + strictcheck + token + appkey)（注：“+”号为合并意思，不包含在被加密的字符串中, **strictcheck填1即可**），输出32位大写字母；</br>当**encryptionalgorithm="RSA"**，业务端RSA私钥签名（appid+token）, 服务端使用业务端提供的公钥验证签名（公钥可以在开发者社区配置）。 |
...
| msisdn              | 可选 | 2    | string   | 解密后的手机号，解密用AES算法，具体算法可以在QQ交流群下载AES.java文件，如果发送请求的idtype为0，则解密的key为sourcekey，如果发送请求的idtype为1，则解密的key为appkey                                       |
...
| 102207 | 获取的中间件值错误                                           |调用SDK清除缓存方法重新获取token|
| 102208 | 参数错误                                                     |
| 102209 | 没有sim卡                                                    |
| 102210 | 不支持短信发送                                               |
| 102211 | 短信验证码验证成功后返回随机码为空                           |
| 102222 | http响应头中没有结果码                                       |
| 102223 | 数据解析异常   |检查发送参数是否正确并且格式是否为json                                              |
| 102299 | other failed                                                 |
| 102302 | 调用service超时                                              |
| 102303 | 用户名为空                                                   |
| 102304 | 密码为空                                                     |
...
| 200014 | 手机号码格式错误                                             |
| 200015 | 短信验证码格式错误                                           |
| 200016 | 更新KS失败                                                   | 调用SDK清除缓存方法重新获取token|
| 200017 | 非移动卡不支持短信上行                                       |
| 200018 | 不支持网关登录                                               |
| 200019 | 不支持短信验证码登录                                         |
| 200020 | 用户取消认证                                                 |
...
| 200033 | 复用中间件获取Token失败                                      | 中间件数据被破坏，调用SDK的清除缓存方法之后重新调用获取token方法即可|
| 200034 | 预取号token失效                                              |
|200036  | 预取号失败                                                   |
...
| 103116     | token验证 sqn错误                          |token只能验证一次，不能多次校验，请重新获取token再校验|
| 103117     | mac异常                                    |
...
| 103138     | appid已存在                                |
| 103139     | sourceid已存在                             |
| 103200     | 不需要更新ks错误                           | 调用SDK清除缓存方法重新获取token|
