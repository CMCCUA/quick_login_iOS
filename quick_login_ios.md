...
继承`UAAuthViewController`类并创建其子类控制器，进行授权页面设计布局和提供动画效果。

**第三步：取号**

调用取号接口`getPhoneNumberWithTimeout`，获取手机号码掩码。

**第四步：授权**

第三步成功后，调用授权登录接口`getAuthorizationWithAuthViewController`，将第二步中用户创建的`UAAuthViewController`的子类控制器传入该接口，获取token和openId。
...
第二步成功后，调用授权登录接口`getAuthorizationWithAuthViewController`，authVC传入nil值，获取token。

**第四步：号码校验**

客户端将SDK返回的token上传到应用服务器，应用服务器调用`本机号码校验`接口，获取用户手机号码。
...
在应用弹出授权页的情况下，调用本方法可以成功获取取号凭证token。如果开发者需要**获取用户完整的手机号码**，调用该方法时，需要将正在运行的授权页面`ViewController`传入并获取相对应的token；如果开发者需要做**本机号码校验**，调用该方法时，`ViewController`参数传nil即可。
...
1、页面必须包含登录按钮，授权登录方法`getAuthorizationWithAuthViewController:completion:`必须通过触发该按钮时调用；
