...
用于使用短信验证码登录，客户端单独调起SDK的短信验证码登录界面（下图），用户手动输入电话号码，获取短信验证码登录，登录成功后获取token和openID等。如果2.4 短信验证码登录开关关闭，此方法不可调用。业务方如果需要获取用户的手机号码，需再次调取3.1接口来换取用户的手机号码等信息。
...
该方法用于一键获取token，用户不需要有任何的界面操作，调用该方法不会弹出任何窗口。
...
| 参数     | 类型   | 说明                                       | 是否必填   |
| ------ | ---- | ---------------------------------------- | ------ |
| enable | BOOL | YES时显式登录取号失败会跳转至短信验证码界面</br>NO时显式登录取号失败不会跳转到短信验证码界面，无法切换账号登录 | 是</br> |
...
| sign          |  1   | string |  必选  | 签名，MD5（appid+version+msgid+systemtime+strictcheck+token+appkey)
（注：“+”号为合并意思，不包含在被加密的字符串中） |
