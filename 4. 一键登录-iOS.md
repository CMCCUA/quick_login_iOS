...
`TYRZBaseApi -> customInit:appKey:`

```objective-c
+ (void)customInit:(NSString *)appID appKey:(NSString *)appKey;
```
...
该方法用于配置是否打开短信验证码服务，默认短信验证码服务是打开状态。SDK在两种情况会使用短信验证码登录：1、一键登录（网关取号）失败后，自动跳转到短验页面；2、在授权页面右上角点击切换账号（见下图）后，用户可以选择使用非本机号码通过短信验证码登录账号。当该方法的属性设置为NO时，将隐藏下图中的`切换账号`按钮，同时，一键登录失败后将不会跳转到短信验证码页面。
...
**功能**
用户自定义的登录logo，如下图3，尺寸要求为：宽85px，高89px
