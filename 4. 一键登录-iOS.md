...
1. 在Xcode中找到`TARGETS-->Build Setting-->Linking-->Other Linker Flags`在这选项中需要添加`-ObjC`


1. TARGETS-->Build Setting-->搜索框中搜索"BitCode"选项,并且将该选项的属性设置为NO


1. 添加bundle资源包 TARGETS -->Build Phases -->Copy Bundle Resources --> 点击 "+" --> Add Other --> TYRZSDK.frameWork --> Resource.bundle -->Open 即可
...
| UAPageNavRightItem                   | UIButton     | 用户自定义导航栏右侧按钮，可通过嵌套在UAAuthPage或UASMSPage分别配置授权页或短验页的功能 | 是     | 2                        |      |
...
| UAPageContentPhoneNumberClearImage | UIImage | 用户自定义号码输入框清除图标 | 是 | 2（如需自定义，必须嵌套在UASMSPage） | |
| UAPageContentSMSCodeBGColor          | UIColor      | 用户自定义短验输入框底色                             | 是     | 2（如需自定义，必须嵌套在UASMSPage）  |      |
| UAPageContentAccountSwitchHidden     | NSNumber     | 用户控制切换账号显示或隐藏，YES隐藏，NO显示                 | 是     | 2（如需自定义，必须嵌套在UAAuthPage） |      |
| UAPageContentLoginButtonBGColor      | UIColor      | 用户自定义登录按钮的底色                             | 是     | 2                        |      |
| UAPageContentLoginButtonUnableBGColor | UIColor | 用户自定义登录按钮不可点击的底色 | 是 | 2（如需自定义，必须嵌套在UASMSPage） | |
| UAPageContentLoginButtonBGImage      | UIImage      | 用户自定义登录按钮背景图                             | 是     | 2                        |      |
| UAPageContentLoginButtonUnableBGImage | UIImage | 用户自定义登录按钮不可点击背景图 | 是 | 2（如需自定义，必须嵌套在UASMSPage） | |
...
| UAPageContentLoginButtonTitle        | NSString     | 用户自定义登录按钮文字                              | 是     | 1或者2，层级为1时，授权页和短验页标题一样   |      |
...
//此处将自定义的视图加进对应页面的View
        if (customAreaView[UAAuthPage]) { //UAAuthPage为一键登录页面的键名（只显示一个手机号码，无验证码输入）
        ...
        if (customAreaView[UASMSPage]) { //UASMSPage为验证码页面的键名（可输入手机号和短信验证码）
            
            UIView *smsView = customAreaView[UASMSPage];
            
            [self customShareButtonsWithView:smsView];
        }
